...
pub async fn install(
    ...
    cfg: &ServerConfig,
) -> Result<AutoModpackStatus, String> {
...
pub async fn generate_client_pack(
    ...
    crate::helpers::download_to_file(&app, &asset.browser_download_url, &tmp_jar, &asset.name)
        .await?;
    ...
    let readme = format!(
        "How to join {server_name}\n\
         ...
         \n\
         — Generated by Lbby\n",
        server_name = if cfg.server_name.trim().is_empty() {
            "this server".into()
        } else {
    ...
    let written = tokio::task::spawn_blocking(move || -> Result<u64, String> {
        ...
        // The mod jar.
        zip.start_file(&asset_name_for_zip, opts)
            .map_err(|e| e.to_string())?;
